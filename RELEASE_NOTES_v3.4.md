...
- Убран экспериментальный поток "Mouse Pass-through", который вызывал залипание мыши.
- Теперь мышь работает **нативно** (без фильтров), а бот просто нажимает кнопки когда нужно.
- Больше не нужно открывать Диспетчер задач, чтобы разблокировать мышь.
